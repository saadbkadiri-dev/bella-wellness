...
## Execution Rules
1. **Plan First:** Before writing any code, generate an `implementation_plan.md` detailing the component structure, file modifications, and design approach. Wait for approval.
2. **Component-Driven:** Keep components modular (e.g., `<HeroSection>`, `<ServiceCard>`, `<PillarCard>`).
3. **Strict Copy Adherence:** Use the exact copy provided in the attached content files. Do not rewrite the brand voice unless necessary for HTML formatting.
4. **No CTAs:** Do not generate any `<button>` elements or `href` links that lead to booking, pricing, or registration.
5. **Responsive:** Ensure perfect responsiveness across all devices.
